SUBROUTINE VelocityBC_colocated(uuB,vvB,wwB,NXm,NYm,NZm) !INOUT  
!_______________________________________________________________________________________

USE COMDAT
INTEGER(4)                                ,INTENT(IN)    :: NXm,NYm,NZm
REAL(8),DIMENSION(0:NXm+1,0:NYm+1,0:NZm+1),INTENT(INOUT) :: uuB,vvB,wwB
...
IF(VBC(1)>0) THEN ! Side Slip 
        !---------------------------------- uuB --------
        DO i=0,NXm+1 ;DO j=0,NZm+1
        uuB(i,NYm+1,j)  =     uuB(i,NYm,j) 
        uuB(i,0,j)      =     uuB(i,1,j)
        ENDDO;ENDDO
        !---------------------------------- wwB --------
        DO j=0,NZm+1 ;DO i=0,NXm+1
...
IF(VBC(3)>0) THEN ! Left Slip 
        !---------------------------------- vvB --------
        DO m=0,NYm+1 ;DO j=0,NZm+1
        vvB(0,m,j)      =     vvB(1,m,j)
        ENDDO;ENDDO
        !---------------------------------- wwB --------
        DO j=0,NZm+1 ;DO m=0,NYm+1
        wwB(0,m,j)      =     wwB(1,m,j)
        ENDDO;ENDDO
ELSE ! Left Nonslip
        !---------------------------------- vvB --------
...
ENDSUBROUTINE VelocityBC_colocated