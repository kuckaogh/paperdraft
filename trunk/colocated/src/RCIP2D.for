...
C     *******************************************************
	...
	REAL*8 F,FX,FY,U,V
	REAL*8 FN,FXN,FYN
	REAL*8 theta,au,av,rxd,ryd,ddx,isx,ddy,jsy,xi,yi,sx,sy,fxj
	...
	A3=CY2*FXN(I,J+JSY)/DDY+CX1*FYN(I+ISX,J)/DDX
     1    +C1*FN(I,J+JSY)/DDY+C2*FN(I+ISX,J)/DDX
     2    +(FN(I,J)-(1.+C1*DDX+C2*DDY)*FN(I+ISX,J+JSY))/DDX/DDY  
     3    +A6*DDX*DDX/DDY+A7*DDY*DDY/DDX+A8*DDX/DDY+A9*DDY/DDX
...
150	CONTINUE 
      ...
      DO 200 J=1,NY
        FX(I,J) = FXN (I,J)
     1                    -FXN (I,J)*(U(I+1,J)-U(I-1,J))*0.5*DT/DX
     1                    -FYN (I,J)*(V(I+1,J)-V(I-1,J))*0.5*DT/DX
        FY(I,J) = FYN (I,J)
     1                    -FXN (I,J)*(U(I,J+1)-U(I,J-1))*0.5*DT/DY
     1                    -FYN (I,J)*(V(I,J+1)-V(I,J-1))*0.5*DT/DY
200	CONTINUE 
